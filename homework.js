...
function faveFood(person){
    for(let item in person){
        if(Array.isArray(person[item])){
            for(i=0; i<item.length; i++){
                if(typeof person[item][i] === 'object'){
                    for(let shakes in person[item][i]){
                        console.log(`${item}: ${shakes}: ${person[item][i][shakes]}`)
                    }
                }else{
                    console.log(`${item}: ${person[item][i]}`)}
                }
        }else{
